...
namespace VHBurguer.Repositories
{
    public class CategoriaRepository : ICategoriaRepository
    {
        ...
        {
            Categoria categoria = _context.Categoria.FirstOrDefault(categoriaAux => categoriaAux.CategoriaID == id);
        ...
        public bool NomeExiste(string nome, int? categoriaIdAtual = null)
        {
            ...
            if (categoriaIdAtual.HasValue)
            {
                ...
                // exemplo -> SELECT * FROM Categroria WHERE CategoriaID != 5
                consulta = consulta.Where(categoriaAux => categoriaAux.CategoriaID != categoriaIdAtual.Value);
            }

            // Verifica se existe alguma catagoria com o mesmo nome
            // retorna true se encontrar ou false se não existir
            return consulta.Any(consultaAux => consultaAux.Nome == nome);
        }
